...
def process():
    ...
    # Delete the rows which the type of their review features are not string
    ...
    df_train.drop(
        ['app_name', 'review', 'votes', 'date', 'label', 'label_name', 'Words Per Review', 'category', 'num_reviews',
         'price', 'rating'], axis=1, inplace=True)
    df_val.drop(
        ['app_name', 'review', 'votes', 'date', 'label', 'label_name', 'Words Per Review', 'category', 'num_reviews',
         'price', 'rating'], axis=1, inplace=True)

    # Convert Dataframes to texts
    np.savetxt(r'../Datasets/Texts/train.txt', df_train.values, fmt='%s')
    np.savetxt(r'../Datasets/Texts/validation.txt', df_val.values, fmt='%s')